      ...
      <div className="bg-white rounded-2xl w-full max-w-sm shadow-2xl">
        <div className="flex items-center justify-between p-6 border-b border-slate-100">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-semibold text-slate-800">Configurar {modeLabel}</h2>
          </div>
          ...
          <div className="p-6 space-y-5">
            <label className="flex items-center justify-between cursor-pointer">
              <div>
                ...
                <p className="text-xs text-slate-400">Inclui termos já dominados (status 6)</p>
              </div>
              <div
                onClick={() => setConfig({ includeMaxStatus: !config.includeMaxStatus })}
                className={`relative w-10 h-6 rounded-full transition-colors cursor-pointer ${config.includeMaxStatus ? "bg-indigo-600" : "bg-slate-300"}`}
              >
            ...
            <label className="flex items-center justify-between cursor-pointer">
              <div>
                <p className="text-sm font-medium text-slate-700">Mostrar cronômetro</p>
                <p className="text-xs text-slate-400">Exibe tempo decorrido durante o jogo</p>
              </div>
              <div
                onClick={() => setConfig({ showTimer: !config.showTimer })}
                className={`relative w-10 h-6 rounded-full transition-colors cursor-pointer ${config.showTimer ? "bg-indigo-600" : "bg-slate-300"}`}
              >
            ...
            <div>
              <label className="text-sm font-medium text-slate-700">
                ...
                <span className="text-slate-400 font-normal">({maxCount} disponíveis)</span>
              </label>
              <input
                type="number"
                min={MIN_GAME_TERMS}
